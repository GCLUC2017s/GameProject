...
void CExplain::Init(){
	...
	mpTextBox->load(FILE_TEXTBOX"white_window.tga");	//テクスチャファイル読み込み
...
CExplain::CExplain() : mpTexture(0),patternflag(false){
	...
	mText1.SetVertex(-5.0f, -2.5f, 5.0f, -5.5f);//表示範囲
	mText1.SetColor(1.0f, 1.0f, 1.0f, 1.0f);

	mText2.SetVertex(-5.0f, -2.5f, 5.0f, -5.5f);//表示範囲
	mText2.SetColor(1.0f, 1.0f, 1.0f, 1.0f);

	mText3.SetVertex(-5.0f, -2.5f, 5.0f, -5.5f);//表示範囲
	mText3.SetColor(1.0f, 1.0f, 1.0f, 1.0f);

	mTextBox.SetVertex(-7.0f, -2.0f, 7.0f, -6.0f);//表示範囲
...
void CExplain::Render(CVector2 pos) {
		...
		mText1.position = pos;
		mText1.position.y = DISP_Y / 22;

		mText2.position = pos;
		mText2.position.y = DISP_Y / 22;

		mText3.position = pos;
		mText3.position.y = DISP_Y / 22;

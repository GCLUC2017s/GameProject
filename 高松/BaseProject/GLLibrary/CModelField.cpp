...
//	while (m_isLoading) { Sleep(1); }
	...
	m_minX = (int)((c.x - showLength) / m_header.cutLength);
	m_maxX = (int)((c.x + showLength) / m_header.cutLength);
	m_minZ = (int)((c.z - showLength) / m_header.cutLength);
	m_maxZ = (int)((c.z + showLength) / m_header.cutLength);

	m_minX = _clamp(m_minX, 0, (int)m_header.cutX-1);
	m_minZ = _clamp(m_minZ, 0, (int)m_header.cutZ-1);
	m_maxX = _clamp(m_maxX, 0, (int)m_header.cutX-1);
	m_maxZ = _clamp(m_maxZ, 0, (int)m_header.cutZ-1);
